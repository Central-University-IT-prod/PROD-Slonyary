...
async def get_user(user_id: int):
    async with db_session_manager() as session:
        query = sa.select(User.id).where(User.telegram_id == user_id)
...
async def add_user(user_id: int,
    ...
    async with db_session_manager() as session:
        query = sa.insert(User).values(telegram_id=user_id,
                                       username=username,
                                       name=name)
...
async def add_channel(channel_id: int,
                      ...
                      username: Optional[str] = None):
    async with db_session_manager() as session:
        query = sa.insert(TgChannel).values(channel_id=channel_id,
                                            owner_id=owner_id,
                                            username=username,
                                            title=title,
                                            photo_url=photo_url)
